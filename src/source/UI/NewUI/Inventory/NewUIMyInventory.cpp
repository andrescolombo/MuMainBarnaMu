...
bool CNewUIMyInventory::EquipItem(int iIndex, std::span<const BYTE> pbyItemPacket)
{
    ...
    // Inject Fenrir Plasma Storm skill client-side when equipping any Fenrir variant.
    // ReceiveMagicList may arrive before inventory on login, so we also inject here
    // to guarantee the skill is present regardless of packet ordering.
    if (pTempItem->Type == ITEM_HORN_OF_FENRIR && pTempItem->HasSkill)
    {
