...
void ReceiveMagicList(const BYTE* ReceiveBuffer)
{
    ...
    else
    {
        ...
        // Inject Fenrir Plasma Storm client-side: the server DB may not have HasSkill
        // set for Horn of Fenrir items, so we mirror the pattern used for Earthshake.
        // All three visual variants (Black/Blue/Red) share ITEM_HORN_OF_FENRIR type.
        {
            const ITEM* pHelperEquip = &CharacterMachine->Equipment[EQUIPMENT_HELPER];
            if (pHelperEquip->Type == ITEM_HORN_OF_FENRIR && pHelperEquip->HasSkill)
            {
                CharacterAttribute->Skill[AT_SKILL_PLASMA_STORM_FENRIR] = AT_SKILL_PLASMA_STORM_FENRIR;
            }
        }
    }
...
void ReceiveMuHelperConfigurationData(std::span<const BYTE> ReceiveBuffer)
{
    ...
    config.bAutoAcceptFriend = MUHelper::g_MuHelper.GetConfig().bAutoAcceptFriend;
    config.bAutoAcceptGuild = MUHelper::g_MuHelper.GetConfig().bAutoAcceptGuild;
    config.bUseSelfDefense = MUHelper::g_MuHelper.GetConfig().bUseSelfDefense;
...
void ReceiveCreatePlayerViewportExtended(std::span<const BYTE> ReceiveBuffer)
{
    ...
    {
        // PK state is packed in the low nibble of RotationAndHeroState.
        // Valid values are 0–PVP_MURDERER2 (6).  Out-of-range nibbles (7–15)
        // are treated as neutral to prevent a newly spawned player from
        // appearing as a murderer before ReceivePK() arrives with the real state.
        const BYTE pkNibble = Data->RotationAndHeroState & 0xf;
        c->PK = (pkNibble <= PVP_MURDERER2) ? pkNibble : PVP_NEUTRAL;
        // Keep the murderer-level display flag consistent with the PK state so
        // there is no visual mismatch between PK and Level at spawn.
        c->Level = (c->PK >= PVP_MURDERER2) ? 1 : 0;
    }
...
void ReceiveAction(const BYTE* ReceiveBuffer, int Size)
{
    ...
    case AT_ATTACK2:
        ...
        else if (!IsMonster(c) && MUHelper::g_MuHelper.GetConfig().bUseSelfDefense)
        {
            MUHelper::g_MuHelper.AddPvpAttacker(Key);
        }
        AttackPlayer = Index;
        break;
...
void ReceiveParty(const BYTE* ReceiveBuffer)
{
    ...
    switch (MUHelper::g_MuHelper.GetConfig().iPartyRequestMode)
    {
    case MUHelper::PARTY_REQUEST_AUTO:
        SocketClient->ToGameServer()->SendPartyInviteResponse(true, PartyKey);
        return;
    case MUHelper::PARTY_REQUEST_OFF:
...
void ReceiveGuild(const BYTE* ReceiveBuffer)
{
    ...
    if (MUHelper::g_MuHelper.GetConfig().bAutoAcceptGuild)
    {
        g_ConsoleDebug->Write(MCD_RECEIVE, L"[MuHelper] Auto-accepting guild invite from key %d", GuildPlayerKey);
...
void ReceiveRequestAcceptAddFriend(const BYTE* ReceiveBuffer)
{
    ...
    if (MUHelper::g_MuHelper.GetConfig().bAutoAcceptFriend)
    {
        SocketClient->ToGameServer()->SendFriendAddResponse(0x01, szName);
        return;
    }
