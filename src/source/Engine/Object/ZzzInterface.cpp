...
bool ElfHasInfinityArrow()
{
    if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) != CLASS_ELF)
        return false;
    return g_isCharacterBuff((&Hero->Object), eBuff_InfinityArrow);
}

bool CheckArrow()
{
    if (ElfHasInfinityArrow())
        return true;
...
void AttackElf(CHARACTER* c, int Skill, float Distance)
{
    ...
    case AT_SKILL_MULTI_SHOT:
    {
        ...
        if (gCharacterManager.GetEquipedBowType_Skill() == BOWTYPE_NONE && !ElfHasInfinityArrow())
            return;
        o->Angle[2] = CreateAngle2D(o->Position, c->TargetPosition);
...
int SelectCharacter(BYTE Kind)
{
    ...
    for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
    {
        CHARACTER* c = &CharactersClient[i];
        OBJECT* o = &c->Object;
        if (o->Kind != 0 && (Kind & o->Kind) != 0 && o->Live && o->Visible && o->Alpha > 0.f)
        {
            o->LightEnable = true;
            switch (c->Level)
            {
