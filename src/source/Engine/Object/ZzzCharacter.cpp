...
void CreateCharacterPointer(CHARACTER* c, int Type, unsigned char PositionX, unsigned char PositionY, float Rotation)
{
    ...
    c->ExtendState = 0;
    c->ExtendStateTime = 0;
    // Default to hero's Gens influence so the strife-map attack check
    // (c->m_byGensInfluence != Hero->m_byGensInfluence) doesn't fire before
    // the real Gens viewport packet arrives.  The packet overwrites this.
    c->m_byGensInfluence = (Hero != nullptr && Hero != c) ? Hero->m_byGensInfluence : 0;
    c->GuildStatus = -1;
    c->GuildType = 0;
