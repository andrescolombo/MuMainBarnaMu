...
- Mu Helper basic-attack fallback and pickup unstick (`src/source/MUHelper/MuHelper.cpp`, `src/source/MUHelper/MuHelper.h`, `src/source/MUHelper/MuHelperData.h`, `src/source/Engine/Object/ZzzInterface.h`, `src/source/UI/NewUI/NewUIMuHelper.cpp`):
  - Symptom 1: with no skill configured in Mu Helper, or when the configured skill could not be cast (insufficient mana / stamina / requirements / safe-zone gate), the helper would stand in front of the target doing nothing instead of falling back to a basic weapon/hand attack.
  - Symptom 2: when a monster was standing on top of a dropped item, `ObtainItem()` kept walking toward the blocked tile forever; `Attack()` never ran because the obtain step returned `0` each tick, so the helper idled.
  - Fix 1 (basic-attack fallback): `CMuHelper::Attack()` now preflights the selected skill through `CanExecuteSkill()`; if that returns false or `SelectAttackSkill()` resolves to `AT_SKILL_UNDEFINED`, the helper calls a new `SimulateBasicAttack(iTarget)` which replicates the main-loop melee/ranged handoff (`SelectedCharacter`, `Attacking=1`, `MovementType=MOVEMENT_ATTACK`, `Action()` → `SendHitRequest`). Range is picked per equipment (1.8 default, 2.2 spear, 6.0 bow). Combo mode is intentionally not changed.
  - Fix 2 (pickup unstick): `ObtainItem()` now (a) defers without blacklisting when `IsMonsterOnTile(TargetX, TargetY)` reports a live monster on the drop's tile so `Attack()` runs that same tick to clear the blocker, (b) hard-skips into a session skip-set when `PathFinding2` returns no path, and (c) hard-skips after `MAX_OBTAIN_STUCK_TICKS = 15` (≈3s at the ~5 Hz `WorkLoop` cadence) of failed tile-reach attempts. `DeleteTarget()` clears the skip-set on every monster removal so drops that were blocked by a now-dead mob get re-armed; `DeleteItem()` also clears the corresponding entry when a drop disappears from the world.
  - Config + UI: new `ConfigData::bFallbackBasicAttack` (default `true`) lets users toggle the fallback in the Mu Helper Misc tab via a new `CHECKBOX_ID_FALLBACK_BASIC_ATTACK` ("Basic Attack Fallback"), registered for all classes. Stored client-only; the `PRECEIVE_MUHELPER_DATA` packet was deliberately not extended to preserve server compatibility.
  - Header housekeeping: `ZzzInterface.h` now declares `CanExecuteSkill` and exposes `extern int ActionTarget;` so `MUHelper` can drive the basic-attack handoff without redeclaring globals; `MuHelper.cpp` includes `Character/CharacterManager.h` for the equipment lookup.
